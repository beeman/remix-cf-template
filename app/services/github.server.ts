...
export async function getFileContent(options: {
	...
	const file: Endpoints['GET /repos/{owner}/{repo}/contents/{path}']['response']['data'] = await resposne.json()
...
export async function getFileContentWithCache(context: AppLoadContext, path: string): Promise<string> {
